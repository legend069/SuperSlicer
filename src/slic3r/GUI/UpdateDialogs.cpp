    ...
    for (const auto &update : updates) {
        ...
        if (!update.comment.empty()) {
            versions->Add(new wxStaticText(
                this, wxID_ANY,
                _(L("Comment:"))) /*, 0, wxALIGN_RIGHT*/); // uncoment if align to right (might not look good if 1
                                                           // vedor name is longer than other names)
            auto *update_comment = new wxStaticText(this, wxID_ANY, from_u8(update.comment));
            update_comment->Wrap(CONTENT_WIDTH * wxGetApp().em_unit());
                ...
                wxICON_ERROR)
{
    auto *text = new wxStaticText(
        this, wxID_ANY,
        wxString::Format(_(L("This version of %s is not compatible with currently installed configuration bundles.\n"
                             "This probably happened as a result of running an older %s after using a newer one.\n\n"

                             "You may either exit %s and try again with a newer version, or you may re-run the "
                             "initial configuration. "
                             "Doing so will create a backup snapshot of the existing configuration before installing "
                             "files compatible with this %s.")) +
                             "\n",
...
MsgDataLegacy::MsgDataLegacy() : MsgDialog(nullptr, _(L("Configuration update")), _(L("Configuration update")))
{
    auto *text = new wxStaticText(
        ...
        from_u8((boost::format(
                     _utf8(L("%s now uses an updated configuration structure.\n\n"
    ...
    auto *link = new wxHyperlinkCtrl(this, wxID_ANY,
                                     wxString::Format("%s 1.40 configuration update", SLIC3R_APP_NAME),
...
AppUpdateAvailableDialog::AppUpdateAvailableDialog(const Semver& version_current, const Semver& version_online, bool from_user)
    : MsgDialog(nullptr, _L("App Update available"), wxString::Format(_L("New version of %s is available. \nDo you want to download it?"), SLIC3R_APP_NAME)) {
        ...
        versions->Add(new wxStaticText(this, wxID_ANY, version_current.to_string()));
        versions->Add(new wxStaticText(this, wxID_ANY, _L("New Version:")));
        versions->Add(new wxStaticText(this, wxID_ANY, version_online.to_string()));
...
#ifndef __linux__
    installing_info = new wxStaticText(this, wxID_ANY,
                                       _L("After finished download explorer will be opened, extract the zip and replace the old version with the new one."));
    content_sizer->Add(installing_info);
#endif
