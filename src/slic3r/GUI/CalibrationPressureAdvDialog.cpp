...
struct ExtrusionSettings {// think a struct is better instead of all the maps ?
    ...
    for (size_t i = 0; i < nb_runs; i++) {
        items.emplace_back((boost::filesystem::path(Slic3r::resources_dir()) / "calibration" / "filament_pressure" /
                            "base_plate.3mf")
                               .string());
    }
    ...
    double filament_max_overlap   = filament_config->get_computed_value("filament_max_overlap",
                                                                      0); // maybe check for extruderID ?
    ...
    if (extrusion_role == "Verify") {
        ...
        for (int i = 0; i < sizeof(choice_extrusion_role) / sizeof(choice_extrusion_role[0]); i++) {
            if (er_width_ToOptionKey.find(extrusion_role) != er_width_ToOptionKey.end()) {
                er_width = print_config->get_abs_value(
                    er_width_ToOptionKey[extrusion_role].c_str(),
                    nozzle_diameter); // look at maps at match speed/width ect to the selecter ER role
                er_speed   = print_config->get_abs_value(er_speed_ToOptionKey[extrusion_role].c_str(),
                                                       nozzle_diameter); // need to load this here??
                er_accel   = print_config->get_abs_value(er_accel_ToOptionKey[extrusion_role].c_str(),
                                                       nozzle_diameter); // need to load this here??
                er_spacing = print_config->get_abs_value(er_spacing_ToOptionKey[extrusion_role].c_str(),
                                                         nozzle_diameter);

                // potential BUG if any of the values are 0 everything else would fail, need to pull the default value
                // too and assign that?
                if (er_width == 0) {
                    er_width = default_er_width;
                }
                ...
                er_width = print_config->get_abs_value(
                    ...
                    nozzle_diameter); // used for gapfill_width/bridges selection. TODO: add the bits for this here
                                      // since gapfill/bridges need special calculations
                er_width = er_width * 100 / nozzle_diameter;
                er_width = std::round(er_width * 100.0) / 100.0; // Change number to percentage and round
            }
        }
    ...
    double er_width_to_scale = magical_scaling(nozzle_diameter, er_width, filament_max_overlap, spacing_ratio,
                                               spacing_ratio_external, base_layer_height, er_spacing);
    //-- magical scaling
    ...
    if (nozzle_diameter_str.back() ==
        '.') { // if nozzle_diameter_str broke fix it by adding '0' to end, prob not needed?
        nozzle_diameter_str += '0';
    }

    /*size_t decimal_pos = nozzle_diameter_str.find('.');
    // maybe adjust for this ?
    // some users might have 0.0x nozzle size. if that's the case then they should just need to create the file and it
    should load. ie; 90_bend_0.450.3mf if (decimal_pos != std::string::npos) { size_t non_zero_pos =
    nozzle_diameter_str.find_first_not_of('0', decimal_pos + 2); nozzle_diameter_str.erase(non_zero_pos,
    std::string::npos);
    }*/
    ...
    for (size_t id_item = 0; id_item < nb_runs; id_item++) {
        ...
        // BUG: if first/base layer height are both .02 numbers don't show up when sliced. doesn't happen with
        // windows, it did for linux ?
        ...
        if (extrusion_role == "Verify") {
            ...
            for (const std::string &role :
                 choice_extrusion_role) { // dynamic add and scale each 90bend model per extrusion role.

                if (er_width_ToOptionKey.find(role) != er_width_ToOptionKey.end()) {
                    er_width = std::round(
                                   (print_config->get_abs_value(er_width_ToOptionKey[role].c_str(), nozzle_diameter) *
                                    100 / nozzle_diameter) *
                                   100.0) /
                               100.0;
                    er_spacing = print_config->get_abs_value(er_spacing_ToOptionKey[role].c_str(), nozzle_diameter);
                    er_width_to_scale = magical_scaling(nozzle_diameter, er_width, filament_max_overlap,
                                                        spacing_ratio, spacing_ratio_external, base_layer_height,
                                                        er_spacing);
                    thickness_offset  = nozzle_diameter * er_width_to_scale * 2;

                    add_part(model.objects[objs_idx[id_item]],
                             (boost::filesystem::path(Slic3r::resources_dir()) / "calibration" / "filament_pressure" /
                              "scaled_with_nozzle_size" / bend_90_nozzle_size_3mf)
                                 .string(),
                             Vec3d{-0.8, (initial_90_bend_y / 2) * nb_bends, xyzScale - base_layer_height},
                             Vec3d{er_width_to_scale, er_width_to_scale, z_scale_90_bend});
                    pressure_tower.back().push_back(model.objects[objs_idx[id_item]]);

                    Eigen::Vector3d modelPosition(-0.8, (initial_90_bend_y / 2) * nb_bends,
                                                  xyzScale - base_layer_height);
            ...
            for (int nb_bends = 0; nb_bends < countincrements; nb_bends++) {
                ...
                add_part(model.objects[objs_idx[id_item]],
                         (boost::filesystem::path(Slic3r::resources_dir()) / "calibration" / "filament_pressure" /
                          "scaled_with_nozzle_size" / bend_90_nozzle_size_3mf)
                             .string(),
                         Vec3d{-0.8, double(nb_bends) * (thickness_offset * 2) * 2, xyzScale - base_layer_height},
                         Vec3d{er_width_to_scale, er_width_to_scale, z_scale_90_bend});
                pressure_tower.back().push_back(model.objects[objs_idx[id_item]]);

                Eigen::Vector3d modelPosition(-0.8, (double(nb_bends) * (thickness_offset * 2) * 2),
                                              xyzScale - base_layer_height);
                bend_90_positions.push_back(modelPosition);
            }
        }
        ...
        for (int nb_bends = 0; nb_bends < countincrements; nb_bends++) {
            if (nb_bends == 1 &&
                extrusion_role != "Verify") { // only load once. this onyl determines when the borders get loaded,
                                              // keeping at top of list makes it easier to scroll down to. it can't be
                                              // '0' since it needs the numbers positions!
                ...
                const double magical_transformation_y_pos =
                    10.47; // load a model without moving its pos to find see what it is.the number doesn't seem to
                           // change regardless of layer heights/nozzle size
                ...
                double          numbers_total_width = (number_pos_last.x() + (xy_scaled_number_x / 2)) -
                                             (number_pos_first.x() - (xy_scaled_number_x / 2));
                ...
                double scaled_tb_border_x_percentage = ((scaled_tb_border_x /* + extra_size_x*/) / initial_border_x) *
                                                       100;

                double total_height = (bend_pos_last.y() + (xy_scaled_90_bend_y / 2)) -
                                      (bend_pos_first.y() - (xy_scaled_90_bend_y / 2));
                double scaled_border_y_percentage = ((total_height + extra_size_y) / initial_90_bend_y) * 100;
                double border_scaled_y            = (initial_border_x * (xy_scaled_x * 1.5)) /
                                         initial_90_bend_y; // need to fix for larger nozzle sizes.

                double right_border_pos_x = number_pos_mid.x();
                double top_border_x_pos   = ((number_pos_last.x() + (xy_scaled_number_x / 2)) +
                                           (bend_pos_first.x() - (xy_scaled_90_bend_x / 2))) /
                                          2;
                ...
                add_part(
                    ...
                    (boost::filesystem::path(Slic3r::resources_dir()) / "calibration" / "filament_pressure" /
                     "pa_border.3mf")
                        .string(),
                    Vec3d{left_border_pos_x + magical_transformation_x_pos,
                          bend_pos_mid.y(), new_z_world_coords}, // need to fix to adjust for nozzle_diameter since it
                                                                 // breaks bottom_solid_layers
                ...
                add_part(model.objects[objs_idx[id_item]],
                         (boost::filesystem::path(Slic3r::resources_dir()) / "calibration" / "filament_pressure" /
                          "pa_border.3mf")
                             .string(),
                         Vec3d{right_border_pos_x + magical_transformation_x_pos, bend_pos_mid.y(),
                               new_z_world_coords},
                         /*scale*/
                         Vec3d{scaled_r_border_x_percentage * 0.01, scaled_border_y_percentage * 0.01,
                               z_scale_factor}); // right border
                ...
                if (enable_top_bottom == true) { // remove later
                    ...
                    add_part(model.objects[objs_idx[id_item]],
                             (boost::filesystem::path(Slic3r::resources_dir()) / "calibration" / "filament_pressure" /
                              "pa_border.3mf")
                                 .string(),
                             Vec3d{top_border_x_pos + magical_transformation_x_pos,
                                   bend_pos_first.y() - (xy_scaled_90_bend_y / 1.8),
                                   new_z_world_coords}, // need to fix to adjust for nozzle_diameter since it breaks
                                                        // bottom_solid_layers
                    ...
                    add_part(model.objects[objs_idx[id_item]],
                             (boost::filesystem::path(Slic3r::resources_dir()) / "calibration" / "filament_pressure" /
                              "pa_border.3mf")
                                 .string(),
                             Vec3d{top_border_x_pos + magical_transformation_x_pos,
                                   bend_pos_last.y() + (xy_scaled_90_bend_y / 1.8),
                                   new_z_world_coords}, // need to fix to adjust for nozzle_diameter since it breaks
                                                        // bottom_solid_layers
            ...
            if (extrusion_role != "Verify") { // possible to load the words for each ER role?
                ...
                const double    magical_transformation_z_pos =
                    0.12; // 0.1 is the transformation value, but set slightly higher so numbers would be "inside"
                          // right border this might be dependant on z_scale_factor
                ...
                for (int j = 0; j < 7;
                     ++j) { // not sure how the code will respond with a positive array list? ie ; 100.2 this moves
                            // decimal point thus breaking the code from loading model since "..3mf" not a real file

                    std::string numered3mfpath = pa_values_string[j] + threemf;

                    if (pa_values_string[j] == '.') {
                        add_part(model.objects[objs_idx[id_item]],
                                 (boost::filesystem::path(Slic3r::resources_dir()) / "calibration" /
                                  "filament_pressure" / "point.3mf")
                                     .string(),
                                 Vec3d{xpos + xy_scaled_number_x + nozzle_diameter, ypos_point,
                                       z_scaled_model_height - magical_transformation_z_pos},
                                 Vec3d{xyzScale * er_width_to_scale, xyzScale + (xyzScale / 2), z_scale_factor});

                        Eigen::Vector3d modelPosition(xpos + xy_scaled_number_x + nozzle_diameter +
                                                          magical_transformation_num_x_pos,
                                                      ypos_point,
                                                      z_scaled_model_height - magical_transformation_z_pos);
                        ...
                        add_part(model.objects[objs_idx[id_item]],
                                 (boost::filesystem::path(Slic3r::resources_dir()) / "calibration" /
                                  "filament_pressure" / numered3mfpath)
                                     .string(),
                                 Vec3d{xpos + xy_scaled_number_x + nozzle_diameter, ypos,
                                       z_scaled_model_height - magical_transformation_z_pos},
                                 Vec3d{xyzScale * er_width_to_scale, xyzScale * er_width_to_scale, z_scale_factor});

                        Eigen::Vector3d modelPosition(xpos + xy_scaled_number_x + nozzle_diameter +
                                                          magical_transformation_num_x_pos,
                                                      ypos, z_scaled_model_height - magical_transformation_z_pos);
                        number_positions.push_back(modelPosition);
    ...
    /*if (!autocenter) {
        ...
        model.objects[objs_idx[0]]->translate({ bed_min.x() + bed_size.x() / 2, bed_min.y() + bed_size.y() / 2, 5 *
    xyzScale - 5 });
    }*/
    ...
    new_print_config.set_key_value("complete_objects",
                                   new ConfigOptionBool(false)); // true is required for multi tests on single plate.
    new_print_config.set_key_value("gap_fill_enabled",
                                   new ConfigOptionBool(true)); // should be false?, enabled for testing
    ...
    new_print_config.set_key_value("seam_position",
                                   new ConfigOptionEnum<SeamPosition>(
                                       spRear)); // BUG: should be fixed in 2.7 merge/SS 2.5.59.7, when this is changed
                                                 // the "perimeters & shell" doesn't turn red indicating a change.
    ...
    new_printer_config.set_key_value("before_layer_gcode",
                                     new ConfigOptionString(std::string("{if layer_num == 0} ") + set_advance_prefix +
                                                            std::to_string(first_pa) + " {endif}"));
        ...
        for (ModelObject *part : pressure_tower[i]) { // loop though each part/volume and assign the modifers
            ...
            if (extrusion_role == "Verify") {
                ...
                if (er_width_ToOptionKey.find(er_role) != er_width_ToOptionKey.end()) {
                    er_width = std::round((print_config->get_abs_value(er_width_ToOptionKey[er_role].c_str(),
                                                                       nozzle_diameter) *
                                           100 / nozzle_diameter) *
                                          100.0) /
                               100.0;
            ...
            er_width = (er_width == 0) ? std::round((default_er_width * 100 / nozzle_diameter) * 100.0) / 100.0 :
                                         er_width;
            ...
            model.objects[objs_idx[i]]
                ->volumes[num_part + extra_vol]
                ->config.set_key_value("perimeter_extrusion_width", new ConfigOptionFloatOrPercent(er_width, true));
            model.objects[objs_idx[i]]->volumes[num_part + extra_vol]->config.set_key_value(
                "external_perimeter_extrusion_width",
                new ConfigOptionFloatOrPercent(
                    er_width, true)); // TODO: check widths and ect breaks if any values are in mm/percentage
            model.objects[objs_idx[i]]
                ->volumes[num_part + extra_vol]
                ->config.set_key_value("perimeter_speed", new ConfigOptionFloatOrPercent(er_speed, false));
            model.objects[objs_idx[i]]
                ->volumes[num_part + extra_vol]
                ->config.set_key_value("external_perimeter_speed", new ConfigOptionFloatOrPercent(er_speed, false));
            model.objects[objs_idx[i]]
                ->volumes[num_part + extra_vol]
                ->config.set_key_value("gap_fill_speed", new ConfigOptionFloatOrPercent(er_speed, false));
            model.objects[objs_idx[i]]
                ->volumes[num_part + extra_vol]
                ->config.set_key_value("perimeter_acceleration", new ConfigOptionFloatOrPercent(er_accel, false));
            model.objects[objs_idx[i]]
                ->volumes[num_part + extra_vol]
                ->config.set_key_value("external_perimeter_acceleration",
                                       new ConfigOptionFloatOrPercent(er_accel, false));
            model.objects[objs_idx[i]]
                ->volumes[num_part + extra_vol]
                ->config.set_key_value("gap_fill_acceleration", new ConfigOptionFloatOrPercent(er_accel, false));
            if (extrusion_role == "Verify") {
                model.objects[objs_idx[i]]
                    ->volumes[num_part + extra_vol]
                ...
                model.objects[objs_idx[i]]->volumes[num_part + extra_vol]->config.set_key_value(
                    "region_gcode", new ConfigOptionString(set_advance_prefix + std::to_string(pa_values[num_part]) +
                                                           " ; " + extrusion_role));
            }
            num_part++;
        }
                                                     ...
                                                     double er_spacing)
{
    ...
    double er_width_decimal = er_width * nozzle_diameter /
                              100.0; // models are generated to be default width of x4 lines for the walls ie; 0.4mm
                                     // nozzle is 1.6mm thick walls
    ...
    if (xyzScale > 4) {
        ...
        er_width_to_scale = er_spacing - (nozzle_diameter / 2 *
                                          0.01); // need to scale slightly under to help with models being correct
                                                 // TODO: test more configurations of nozzle sizes/layer heights
