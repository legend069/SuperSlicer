...
{
public:
    SplashScreen(const wxBitmap &bitmap,
                 long splashStyle,
                 int milliseconds,
...
bool GUI_App::on_init_inner() {
    ...
    if (app_config->get_bool("show_splash_screen")) {
        ...
        // make a bitmap with dark grey banner on the left side
        scrn = new SplashScreen(bmp.IsOk() ?
                                    bmp :
                                    SplashScreen::MakeBitmap(
                                        get_bmp_bundle(SLIC3R_APP_KEY, 600)->GetPreferredBitmapSizeAtScale(1.0)),
