///|/ Copyright (c) Prusa Research 2018 - 2023 Vojtěch Bubník @bubnikv, Oleksandra Iushchenko @YuSanka, Lukáš Matěna
///@lukasmatena, Tomáš Mészáros @tamasmeszaros, David Kocík @kocikdav, Enrico Turri @enricoturri1966, Lukáš Hejl
///@hejllukas, Filip Sykala @Jony01, Vojtěch Král @vojtechkral
...
//#endif
        ...
        m_action_line_y_position = logo_and_header_height +
            0.5 * (m_main_bitmap.GetHeight() - margin - credits_height - logo_and_header_height - text_height);
    }
...
// an extension from the provided initial file name and substitutes it with the default extension (the first one in
// the template).
...
static void register_win32_device_notification_event() {
    wxWindow::MSWRegisterMessageHandler(
        WM_DEVICECHANGE, [](wxWindow *win, WXUINT /* nMsg */, WXWPARAM wParam, WXLPARAM lParam) {
            // Some messages are sent to top level windows by default, some messages are sent to only registered
            // windows, and we explictely register on MainFrame only.
    ...
    wxWindow::MSWRegisterMessageHandler(MainFrame::WM_USER_MEDIACHANGED,
                                        [](wxWindow *win, WXUINT /* nMsg */, WXWPARAM wParam, WXLPARAM lParam) {
                                            // Some messages are sent to top level windows by default, some messages
                                            // are sent to only registered windows, and we explictely register on
                                            // MainFrame only.
                                            auto main_frame = dynamic_cast<MainFrame *>(win);
                                            auto plater = (main_frame == nullptr) ? nullptr : main_frame->plater();
                                            if (plater == nullptr)
                                            ...
                                            if (lParam == SHCNE_MEDIAINSERTED || lParam == SHCNE_MEDIAREMOVED) {
                                                ...
                                                if (!SHGetPathFromIDList(pidl, sPath)) {
                                                    BOOST_LOG_TRIVIAL(error)
                                                        << "MediaInserted: SHGetPathFromIDList failed";
                                                    return false;
                                                }
                                            }
                                            switch (lParam) {
    ...
    wxWindow::MSWRegisterMessageHandler(WM_COPYDATA,
                                        [](wxWindow *win, WXUINT /* nMsg */, WXWPARAM wParam, WXLPARAM lParam) {
                                            ...
                                            if (copy_data_structure->dwData == 1) {
                                                ...
                                                Slic3r::GUI::wxGetApp()
                                                    .other_instance_message_handler()
                                                    ->handle_message(boost::nowide::narrow(arguments));
                                            }
                                            return true;
                                        });
}
...
static void generic_exception_handle() {
    ...
    try {
        ...
        wxString errmsg =
            wxString::Format(_L("%s has encountered a localization error. "
...
void GUI_App::post_init() {
    ...
    if (this->is_gcode_viewer()) {
        ...
        if (!this->init_params->load_configs.empty())
            ...
            // FIXME this is not strictly correct, as one may pass a print/filament/printer profile here instead of a
            // full config.
...
void GUI_App::init_app_config() {
    ...
    if (data_dir().empty()) {
        ...
        if (boost::filesystem::exists(boost::filesystem::path{resources_dir()} / ".." / ".." / ".." /
                                      "configuration")) {
    ...
    if (m_app_conf_exists) {
        ...
        if (!error.empty()) {
            ...
            if (is_editor()) {
                throw Slic3r::RuntimeError(format(_u8L("Error parsing %1% config file, it is probably corrupted. "
                                                       "Try to manually delete the file to recover from the error. "
                                                       "Your user profiles will not be affected."
                                                       "\n\n%2%\n\n%3%"),
                                                  SLIC3R_APP_NAME, app_config->config_path(), error));
            } else {
                throw Slic3r::RuntimeError(format(_u8L("Error parsing %1% config file, it is probably corrupted. "
                                                       "Try to manually delete the file to recover from the error."
                                                       "\n\n%2%\n\n%3%"),
                                                  GCODEVIEWER_APP_NAME, app_config->config_path(), error));
            }
        }
    }
...
bool GUI_App::on_init_inner() {
    ...
    if (wxPlatformInfo::Get().GetArchName().substr(0, 2) == "64") {
        RichMessageDialog dlg(nullptr,
                              format_wxstr(_L("You are running a 32 bit build of %s on 64-bit Windows."
                                              "\n32 bit build of %s will likely not be able to utilize all the RAM "
    ...
    // !!! Initialization of UI settings as a language, application color mode, fonts... have to be done before first
    // UI action. Like here, before the show InfoDialog in check_older_app_config()
    ...
    if (m_app_conf_exists) {
        ...
        // No AppConfig exists, fresh install. Always try to copy from an alternate location, don't make backup of the
    ...
    if (app_config->get_bool("show_splash_screen")) {
        ...
        wxString artist;
        if (!file_name.empty()) {
            boost::filesystem::path splash_screen_path = (boost::filesystem::path(Slic3r::resources_dir()) /
    ...
    try {
        // Enable all substitutions (in both user and system profiles), but log the substitutions in user profiles
        // only. If there are substitutions in system profiles, then a "reconfigure" event shall be triggered, which
        // will force installation of a compatible system preset, thus nullifying the system preset substitutions.
    ...
    Bind(wxEVT_IDLE, [this](wxIdleEvent &event) {
        ...
        // and wxEVT_SET_FOCUS before GUI_App::post_init is called) wasn't called before GUI_App::post_init and OpenGL
        // wasn't initialized. Since issue #9774 Where same problem occured on MacOS Ventura, we decided to have this
        // check on MacOS as well.
    ...
    if (const std::string &crash_reason = app_config->get("restore_win_position");
        ...
        InfoDialog dialog(nullptr, format_wxstr(_L("%1% started after a crash"), SLIC3R_APP_NAME),
                          format_wxstr(_L("%5% crashed last time when attempting to set window position.\n"
                                       ...
                                       "<b>" + preferences_item + "</b>", SLIC3R_APP_NAME),
                          true, wxYES_NO);
...
const wxColour GUI_App::get_label_default_clr_system() {
    return dark_mode() ? wxColour(115, 220, 103) : wxColour(255, 255, 255);
}

const wxColour GUI_App::get_label_default_clr_modified() {
    return dark_mode() ? wxColour(253, 111, 40) : wxColour(255, 255, 255);
}

const wxColour GUI_App::get_label_default_clr_default() {
    return dark_mode() ? wxColour(250, 250, 250) : wxColour(255, 255, 255);
}
...
void GUI_App::update_ui_colours_from_appconfig() {
    ...
    m_color_hovered_btn = is_dark_mode ?
        color_from_int(app_config->create_color(0.84f, 0.99f, AppConfig::EAppColorType::Main)) :
...
#ifdef __APPLE__
    // This is a limit imposed by OSX. The way the native button widget is drawn only allows it to be stretched
    // horizontally, and the vertical size is fixed. (see
    // https://stackoverflow.com/questions/29083891/wxpython-button-size-being-ignored-on-osx) But standard height is
    // possible to change using SetWindowVariant method (see
...
void GUI_App::tiled_canvas_dialog() { change_calibration_dialog(nullptr, new CreateMMUTiledCanvas(this, mainframe)); }
...
void GUI_App::load_project(wxWindow *parent, wxString &input_file) const {
    input_file.Clear();
    wxFileDialog dialog(parent ? parent : GetTopWindow(), _L("Choose one file (3MF/AMF):"),
...
bool GUI_App::select_language() {
    ...
    if (index != -1 && (init_selection == -1 || init_selection != index)) {
        ...
        if (this->load_language(new_language_info->CanonicalName, false)) {
            ...
            // 1) Hopefully the language set to wxTranslations by this->load_language(), but that API is weird and we
            // don't want to rely on its
...
bool GUI_App::load_language(wxString language, bool initial) {
    if (initial) {
        ...
        if (!language.empty())
            BOOST_LOG_TRIVIAL(trace) << boost::format("translation_language provided by " SLIC3R_APP_NAME
                                                      ".ini: %1%") %
                    language;
        ...
        }
        {
            ...
            // Use English as an initial language, because of under OSX it try to load "inappropriate" language for
            // wxLANGUAGE_DEFAULT. For example in our case it's trying to load "en_CZ" and as a result PrusaSlicer
            // catch warning message. But wxWidgets guys work on it.
            temp_locale.Init(wxLANGUAGE_ENGLISH);
#else
            temp_locale.Init();
#endif // __WXOSX__
       // Set the current translation's language to default, otherwise GetBestTranslation() may not work (see the
       // wxWidgets source code).
            ...
            // wxLocale::GetSystemLanguage(). The last parameter gets added to the list of detected dictionaries. This
            // is a workaround for not having the English dictionary. Let's hope wxWidgets of various versions process
    ...
    if (!language.empty() && (language_info == nullptr || language_info->CanonicalName.empty())) {
        // Fix for wxWidgets issue, where the FindLanguageInfo() returns locales with undefined ANSII code
        // (wxLANGUAGE_KONKANI or wxLANGUAGE_MANIPURI).
    ...
    if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
        BOOST_LOG_TRIVIAL(trace)
            << boost::format(
                   "The following language code requires right to left layout, which is not supported by %1%: %2%") %
                SLIC3R_APP_NAME % language_info->CanonicalName.ToUTF8().data();
        language_info = nullptr;
    }
    ...
    if (language_info == m_language_info_best || language_info == m_language_info_system) {
        ...
        // Use whatever the operating system recommends, if it the language code of the dictionary matches the recommended
        // language. This allows a Swiss guy to use a German dictionary without forcing him to German locales.
        language_info = m_language_info_best;
    ...
    // FIXME This is a temporary workaround, the correct solution is to switch to "C" locale during file import /
    // export only. wxSetlocale(LC_NUMERIC, "C");
...
void GUI_App::add_config_menu(wxMenuBar *menu) {
    ...
    wxWindowID config_id_base = wxWindow::NewControlId(
        int(ConfigMenuCnt + Slic3r::GUI::get_app_config()->tags().size() * 2));
    ...
    if (is_editor()) {
        ...
        for (const AppConfig::Tag &tag : Slic3r::GUI::get_app_config()->tags()) {
            mode_menu->AppendCheckItem(config_id_base + ConfigMenuCnt + config_menu_idx, _(tag.name),
                                       _(tag.description));
            Bind(
                wxEVT_UPDATE_UI, [this, tag](wxUpdateUIEvent &evt) { evt.Check((get_mode() & tag.tag) == tag.tag); },
                config_id_base + ConfigMenuCnt + config_menu_idx);
            config_menu_idx++;
        }
        ...
        tag_menu = new wxMenu();
        for (const AppConfig::Tag &tag : Slic3r::GUI::get_app_config()->tags()) {
            tag_menu->AppendCheckItem(config_id_base + ConfigMenuCnt + config_menu_idx, _(tag.name),
                                      _(tag.description));
            Bind(
                wxEVT_UPDATE_UI, [this, tag](wxUpdateUIEvent &evt) { evt.Check((get_mode() & tag.tag) == tag.tag); },
                config_id_base + ConfigMenuCnt + config_menu_idx);
    ...
    local_menu->Bind(wxEVT_MENU, [this, config_id_base](wxEvent &event) {
        ...
        case ConfigMenuTakeSnapshot:
            ...
            if (wxString action_name = _L("Taking a configuration snapshot");
                check_and_save_current_preset_changes(action_name,
                                                      _L("Some presets are modified and the unsaved changes will not "
                                                         "be captured by the configuration snapshot."),
                                                      false, true)) {
                ...
                if (dlg.ShowModal() == wxID_OK)
                    if (const Config::Snapshot *snapshot =
                            Config::take_config_snapshot_report_error(*app_config, Config::Snapshot::SNAPSHOT_USER,
    ...
    if (mode_menu != nullptr) {
        auto modefn = [this](ConfigOptionMode mode, wxCommandEvent &) {
            if (get_mode() != mode)
                save_mode(mode);
        };
        int config_menu_idx = 0;
        for (const AppConfig::Tag &tag : Slic3r::GUI::get_app_config()->tags()) {
            mode_menu->Bind(wxEVT_MENU, std::bind(modefn, tag.tag, _1),
                            config_id_base + ConfigMenuCnt + config_menu_idx);
            config_menu_idx++;
        }
        if (tag_menu != nullptr) {
            auto tagfn = [this](ConfigOptionMode mode, wxCommandEvent &) {
                if (get_mode() != mode)
                    save_mode(get_mode() ^ mode);
            };
            for (const AppConfig::Tag &tag : Slic3r::GUI::get_app_config()->tags()) {
                tag_menu->Bind(wxEVT_MENU, std::bind(tagfn, tag.tag, _1),
                               config_id_base + ConfigMenuCnt + config_menu_idx);
                config_menu_idx++;
            }
        }
...
void GUI_App::open_wifi_config_dialog(bool forced, const wxString &drive_path /* = {}*/) {
    ...
    if (!forced && dialog_was_declined) {
        ...
        notification_manager()
            ->push_notification(
                ...
                // TRN Text of hypertext of notification when Slicer starts and usb stick with printer settings ini
                // file is present
                ,
    ...
    if (dialog.ShowModal() == wxID_OK) {
        plater_->get_notification_manager()
            ->push_exporting_finished_notification(file_path,
                                                   boost::filesystem::path(file_path).parent_path().string(), true);
    ...
    for (Tab *tab : tabs_list)
        if (tab->supports_printer_technology(printer_technology) && tab->get_presets()) {
            if (tab->type() == Preset::TYPE_PRINTER) {
                ...
                // active extruder can be changed in a respect to the new loaded configurations, if some filament
                // preset will be modified
...
void GUI_App::open_web_page_localized(const std::string &http_address) {
    ...
    open_browser_with_warning_dialog(wxString(http_address) + wxString(query_pos == std::string::npos ? "?" : "&") +
                                         lng_param,
                                     nullptr, false);
}

// If we are switching from the FFF-preset to the SLA, we should to control the printed objects if they have a
// part(s). Because of we can't to print the multi-part objects with SLA technology.
        ...
        while (std::getline(in_file, gcode_line)) {
            if (in_file.good()) {
                if (boost::starts_with(gcode_line, BEGIN_MASK)) {
                    ...
                    std::string out_filename = out_path + std::to_string(width) + "x" + std::to_string(height) +
                        ".png";
...
bool GUI_App::open_browser_with_warning_dialog(const wxString &url,
    ...
    if (force_remember_choice || app_config->get(option_key).empty()) {
        if (app_config->get(option_key).empty()) {
            RichMessageDialog dialog(parent, _L("Open hyperlink in default browser?"),
                                     format(_L("%1%: Open hyperlink"), SLIC3R_APP_NAME), wxICON_QUESTION | wxYES_NO);
            dialog.ShowCheckBox(_L("Remember my choice"));
            auto answer = dialog.ShowModal();
            launch = answer == wxID_YES;
            if (dialog.IsCheckBoxChecked()) {
                wxString preferences_item = _L("Suppress to open hyperlink in browser");
                wxString msg = format(_L("%1% will remember your choice."), SLIC3R_APP_NAME) + "\n\n" +
                    _L("You will not be asked about it again on hyperlinks hovering.") + "\n\n" +
                    format_wxstr(_L("Visit \"Preferences\" and check \"%1%\"\nto changes your choice."),
                                 preferences_item);

                MessageDialog msg_dlg(parent, msg, format(_L("%1%: Don't ask me again"), SLIC3R_APP_NAME),
    ...
    // and will be shown only when "Suppress to open hyperlink in browser" is ON.
        ...
        MessageDialog dialog(parent, _L("Open hyperlink in default browser?"),
                             format(_L("%1%: Open hyperlink"), SLIC3R_APP_NAME), wxICON_QUESTION | wxYES_NO);
...
void GUI_App::app_updater(bool from_user) {
    ...
    if (opt != "none" || m_check_for_application_update) {
        ...
        // start download
        this->plater_->get_notification_manager()
            ->push_download_progress_notification(GUI::format(_L("Downloading %1%"),
...
void GUI_App::start_download(std::string url) {
    ...
    if (dest_folder.empty() || !boost::filesystem::is_directory(dest_folder)) {
        std::string msg = _u8L("Could not start URL download. Destination folder is not set. Please choose "
