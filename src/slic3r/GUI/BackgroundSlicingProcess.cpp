///|/ Copyright (c) Prusa Research 2018 - 2023 Lukáš Matěna @lukasmatena, Oleksandra Iushchenko @YuSanka, Vojtěch
///Bubník @bubnikv, Pavel Mikuš @Godrak, David Kocík @kocikdav, Enrico Turri @enricoturri1966, Tomáš Mészáros
///@tamasmeszaros, Roman Beránek @zavorka, Vojtěch Král @vojtechkral
...
std::pair<std::string, bool> SlicingProcessCompletedEvent::format_error_message() const {
    ...
    try {
        ...
        error =
            GUI::format(_L("%1% has encountered an error. It was likely caused by running out of memory. "
        ...
        error = ex.what();
        monospace = true;
    } catch (std::exception &ex) { error = ex.what(); } catch (...) {
...
void BackgroundSlicingProcess::process_sla() {
    ...
    if (this->set_step_started(bspsGCodeFinalize)) {
        if (!m_export_path.empty()) {
            ...
            ThumbnailsList thumbnails = this->render_thumbnails(ThumbnailsParams{
                current_print()->full_print_config().option<ConfigOptionPoints>("thumbnails")->get_values(), true,
                true, true, true});
...
// Wrapper for Win32 structured exceptions. Win32 structured exception blocks and C++ exception blocks cannot be mixed
...
unsigned long BackgroundSlicingProcess::call_process_seh(std::exception_ptr &ex) throw() {
    ...
    } __except (is_win32_seh_harware_exception(GetExceptionCode())) { win32_seh_catched = GetExceptionCode(); }
...
void BackgroundSlicingProcess::thread_proc_safe_seh_throw() throw() {
    ...
    if (win32_seh_catched) {
        ...
        try {
            rethrow_seh_exception(win32_seh_catched);
        } catch (...) { wxTheApp->OnUnhandledException(); }
    }
}
#endif // _WIN32
...
Print::ApplyStatus BackgroundSlicingProcess::apply(const Model &model,
                                                   const DynamicPrintConfig &config,
                                                   const DynamicPrintConfig *physical_printer_config) {
    assert(m_print != nullptr);
    assert(config.opt_enum<PrinterTechnology>("printer_technology") == m_print->technology());
    Print::ApplyStatus invalidated = m_print->apply(model, config);
    if (physical_printer_config &&
        (m_print->physical_printer_config().diff(*physical_printer_config).size() != 0 ||
         m_print->physical_printer_config().keys() != physical_printer_config->keys())) {
...
void BackgroundSlicingProcess::prepare_upload() {
    ...
    if (m_print == m_fff_print) {
        ...
        // Make a copy of the source path, as run_post_process_scripts() is allowed to change it when making a copy of
        // the source file (not here, but when the final target is a file).
        ...
        m_sla_print->export_print(source_path.string(), thumbnails,
                                  m_upload_job.upload_data.upload_path.filename().string());
    }

    m_print->set_status(100,
                        GUI::format(_L("Scheduling upload to `%1%`. See Window -> Print Host Upload Queue"),
