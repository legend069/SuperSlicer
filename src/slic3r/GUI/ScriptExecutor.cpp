...
{
public:
    NoDefinitionExceptionEmitLog(const std::string &message) : ConfigurationError(message)
    {
...
void             as_print(std::string &str) { std::cout << str; }
void             as_print_float(float f) { std::cout << f; }

void as_print_int(int i) { std::cout << i; }
...
{
    const ConfigOption *opt = get_coll(key).second;
    if (opt == nullptr || (opt->type() != ConfigOptionType::coInt && opt->type() != ConfigOptionType::coInts &&
                           opt->type() != ConfigOptionType::coEnum))
...
float as_get_float_idx(std::string &key, int idx)
{
    ...
    if (opt->type() == ConfigOptionType::coFloatOrPercent &&
        static_cast<const ConfigOptionFloatOrPercent *>(opt)->percent)
        val *= 0.01;
    if (opt->is_vector()) {
        const ConfigOptionVectorBase *vector = static_cast<const ConfigOptionVectorBase *>(opt);
        if (opt->type() == ConfigOptionType::coFloatsOrPercents &&
            static_cast<const ConfigOptionFloatsOrPercents *>(vector)->get_at(idx).percent)
            val *= 0.01;
...
void _set_float(DynamicPrintConfig &conf, const ConfigOption *opt, std::string &key, int idx, float f_val)
{
    if (opt->type() == ConfigOptionType::coFloat) {
        ...
        if (idx < 0)
            // replace all values
            for (size_t i = 0; i < new_opt->size(); ++i) new_opt->set_at(new_val, i);
        else
...
bool as_is_percent_idx(std::string &key, int idx)
{
    ...
    return (opt->type() == ConfigOptionType::coPercent) || (opt->type() == ConfigOptionType::coPercents) ||
           ...
           (opt->type() == ConfigOptionType::coFloatsOrPercents &&
            ((ConfigOptionFloatsOrPercents *) opt)->get_at(idx).percent);
}
...
void _set_percent(DynamicPrintConfig &conf, const ConfigOption *opt, std::string &key, int idx, float p_val)
{
    ...
    if (opt->type() == ConfigOptionType::coFloat) {
        ...
        if (idx < 0)
            for (size_t i = 0; i < new_opt->size(); ++i) new_opt->set_at(percent_f / 100., i);
        else
        ...
        if (idx < 0)
            for (size_t i = 0; i < new_opt->size(); ++i) new_opt->set_at(FloatOrPercent{percent_f, true}, i);
        else
...
void _set_string(DynamicPrintConfig &    conf,
                 const PresetCollection *pcoll,
...
std::string get_custom_var_option(int preset_type)
{
    if (preset_type <= 0)
        return (current_script->tab()->get_printer_technology() & PrinterTechnology::ptFFF) != 0 ?
                   current_script->tab()->m_preset_bundle->fff_prints.get_edited_preset().config.opt_string(
                       "print_custom_variables") :
                   current_script->tab()->m_preset_bundle->sla_prints.get_edited_preset().config.opt_string(
                       "print_custom_variables");
    else if (preset_type == 1) {
        return (current_script->tab()->get_printer_technology() & PrinterTechnology::ptFFF) != 0 ?
                   current_script->tab()->m_preset_bundle->filaments.get_edited_preset().config.opt_string(
                       "filament_custom_variables", (unsigned int) (0)) :
                   current_script->tab()->m_preset_bundle->sla_materials.get_edited_preset().config.opt_string(
                       "filament_custom_variables", (unsigned int) (0));
    } else
        return current_script->tab()->m_preset_bundle->printers.get_edited_preset().config.opt_string(
...
void set_custom_option(int preset_type, std::string new_value)
{
    ...
    if (preset_type <= 0) {
        ...
        const PresetCollection &coll = (current_script->tab()->get_printer_technology() & PrinterTechnology::ptFFF) !=
                                               0 ?
                                           current_script->tab()->m_preset_bundle->filaments :
                                           current_script->tab()->m_preset_bundle->sla_materials;
        DynamicPrintConfig &    conf = current_script->to_update()[coll.type()];
...
public:
    ...
    ConfigAdapter(const ConfigBase *conf, const ConfigBase *conf_parent) : real_storage(conf)
    {
        this->parent = conf_parent;
    }
...
float as_get_computed_float(std::string &key)
{
    if ((current_script->tab()->get_printer_technology() & PrinterTechnology::ptFFF) != 0) {
        ConfigAdapter fullconfig(
            ...
            new ConfigAdapter(&current_script->tab()->m_preset_bundle->filaments.get_edited_preset().config,
                              new ConfigAdapter(
                                  &current_script->tab()->m_preset_bundle->printers.get_edited_preset().config)));
        try {
        ...
        ConfigAdapter fullconfig(
            ...
            new ConfigAdapter(&current_script->tab()->m_preset_bundle->sla_materials.get_edited_preset().config,
                              new ConfigAdapter(
                                  &current_script->tab()->m_preset_bundle->printers.get_edited_preset().config)));
        try {
            return (float) fullconfig.get_computed_value(key, 0);
...
void as_ask_for_refresh() { current_script->request_refresh(); }
...
void as_back_custom_initial_value(int preset_type, std::string &key)
{
    ...
    if (preset_type <= 0)
        initial_serialized_vars =
            (current_script->tab()->get_printer_technology() & PrinterTechnology::ptFFF) != 0 ?
                current_script->tab()->m_preset_bundle->fff_prints.get_selected_preset().config.opt_string(
                    "print_custom_variables") :
                current_script->tab()->m_preset_bundle->sla_prints.get_selected_preset().config.opt_string(
                    "print_custom_variables");
    else if (preset_type == 1) {
        initial_serialized_vars = (current_script->tab()->get_printer_technology() & PrinterTechnology::ptFFF) != 0 ?
                                      current_script->tab()
                                          ->m_preset_bundle->filaments.get_selected_preset()
                                          .config.opt_string("filament_custom_variables", (unsigned int) (0)) :
                                      current_script->tab()
                                          ->m_preset_bundle->sla_materials.get_selected_preset()
                                          .config.opt_string("filament_custom_variables", (unsigned int) (0));
    } else
        initial_serialized_vars =
            current_script->tab()->m_preset_bundle->printers.get_selected_preset().config.opt_string(
                "printer_custom_variables");
...
void ScriptContainer::init(const std::string &tab_key, Tab *tab)
{
    ...
    if (boost::filesystem::exists(ui_script_file)) {
        ...
        if (m_script_engine.get() == nullptr) {
            ...
            m_script_engine->SetMessageCallback(AngelScript::asFUNCTION(as_message_callback), 0,
            ...
            // if (use_generic) {
            //    r = engine->RegisterGlobalFunction("void print(string & in)", WRAP_FN(print), asCALL_GENERIC);
            //    assert(r >= 0);
            //} else {
            ...
            m_script_engine.get()->RegisterGlobalFunction("void print(string &in)", WRAP_FN(as_print),
                                                          AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void print_float(float)", WRAP_FN(as_print_float),
                                                          AngelScript::asCALL_GENERIC);
            // m_script_engine.get()->RegisterGlobalFunction("void register_key(string &in)",
            // WRAP_FN(as_register_key), AngelScript::asCALL_GENERIC);

            m_script_engine.get()->RegisterGlobalFunction("int get_int_idx(string &in)", WRAP_FN(as_get_int_idx),
                                                          AngelScript::asCALL_GENERIC);

            m_script_engine.get()->RegisterGlobalFunction("void set_int_idx(string &in, int new_val)",
                                                          WRAP_FN(as_set_int_idx), AngelScript::asCALL_GENERIC);

            m_script_engine.get()->RegisterGlobalFunction("float get_nozzle(string &in, int idx)",
                                                          WRAP_FN(as_get_nozzle), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void set_nozzle(string &in, float new_val, int idx)",
                                                          WRAP_FN(as_set_nozzle), AngelScript::asCALL_GENERIC);

            m_script_engine.get()->RegisterGlobalFunction("bool get_bool(string &in)", WRAP_FN(as_get_bool),
                                                          AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void set_bool(string &in, bool new_val)",
                                                          WRAP_FN(as_set_bool), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("int get_int(string &in)", WRAP_FN(as_get_int),
                                                          AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void set_int(string &in, int new_val)",
                                                          WRAP_FN(as_set_int), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("float get_float(string &in)", WRAP_FN(as_get_float),
                                                          AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void set_float(string &in, float new_val)",
                                                          WRAP_FN(as_set_float), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("bool is_percent(string &in)", WRAP_FN(as_is_percent),
                                                          AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void set_percent(string &in, float new_val)",
                                                          WRAP_FN(as_set_percent), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void get_string(string &in, string &out get_val)",
                                                          WRAP_FN(as_get_string), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void set_string(string &in, string &in new_val)",
                                                          WRAP_FN(as_set_string), AngelScript::asCALL_GENERIC);

            // for vector fields
            m_script_engine.get()->RegisterGlobalFunction("int size(string &in)", WRAP_FN(as_size),
                                                          AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void clear(string &in)", WRAP_FN(as_clear),
                                                          AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("bool get_bool_idx(string &in, int idx)",
                                                          WRAP_FN(as_get_bool_idx), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void set_bool_idx(string &in, int idx, bool new_val)",
                                                          WRAP_FN(as_set_bool_idx), AngelScript::asCALL_GENERIC);
            ...
            m_script_engine.get()->RegisterGlobalFunction("float get_float_idx(string &in, int idx)",
                                                          WRAP_FN(as_get_float_idx), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void set_float_idx(string &in, int idx, float new_val)",
                                                          WRAP_FN(as_set_float_idx), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("bool is_percent_idx(string &in, int idx)",
                                                          WRAP_FN(as_is_percent_idx), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void set_percent_idx(string &in, int idx, float new_val)",
                                                          WRAP_FN(as_set_percent_idx), AngelScript::asCALL_GENERIC);
            m_script_engine.get()
                ->RegisterGlobalFunction("void get_string_idx(string &in, int idx, string &out get_val)",
                                         WRAP_FN(as_get_string_idx), AngelScript::asCALL_GENERIC);
            m_script_engine.get()
                ->RegisterGlobalFunction("void set_string_idx(string &in, int idx, string &in new_val)",
                                         WRAP_FN(as_set_string_idx), AngelScript::asCALL_GENERIC);

            m_script_engine.get()->RegisterGlobalFunction("bool get_custom_bool(int, string &in, bool &out)",
                                                          WRAP_FN(as_get_custom_bool), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void set_custom_bool(int, string &in, bool)",
                                                          WRAP_FN(as_set_custom_bool), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("bool get_custom_int(int, string &in, int &out)",
                                                          WRAP_FN(as_get_custom_int), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void set_custom_int(int, string &in, int)",
                                                          WRAP_FN(as_set_custom_int), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("bool get_custom_float(int, string &in, float &out)",
                                                          WRAP_FN(as_get_custom_float), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void set_custom_float(int, string &in, float)",
                                                          WRAP_FN(as_set_custom_float), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("bool get_custom_string(int, string &in, string &out)",
                                                          WRAP_FN(as_get_custom_string), AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void set_custom_string(int, string &in, string &in)",
                                                          WRAP_FN(as_set_custom_string), AngelScript::asCALL_GENERIC);

            m_script_engine.get()->RegisterGlobalFunction("float get_computed_float(string &in)",
                                                          WRAP_FN(as_get_computed_float),
                                                          AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void back_initial_value(string &in)",
                                                          WRAP_FN(as_back_initial_value),
                                                          AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void back_custom_initial_value(int, string &in)",
                                                          WRAP_FN(as_back_custom_initial_value),
                                                          AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("void ask_for_refresh()", WRAP_FN(as_ask_for_refresh),
                                                          AngelScript::asCALL_GENERIC);
            m_script_engine.get()->RegisterGlobalFunction("bool is_enabled(string &in)", WRAP_FN(as_is_enabled),
                                                          AngelScript::asCALL_GENERIC);

#else
            m_script_engine.get()->RegisterGlobalFunction("void print(string &in)", AngelScript::asFUNCTION(as_print),
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("void print_float(float)",
                                                          AngelScript::asFUNCTION(as_print_float),
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("void print_int(int)",
                                                          AngelScript::asFUNCTION(as_print_int),
                                                          AngelScript::asCALL_CDECL);
            // m_script_engine.get()->RegisterGlobalFunction("void register_key(string &in)",
            // AngelScript::asFUNCTION(as_register_key), AngelScript::asCALL_CDECL);

            m_script_engine.get()->RegisterGlobalFunction("bool get_bool(string &in)",
                                                          AngelScript::asFUNCTION(as_get_bool),
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("void set_bool(string &in, bool new_val)",
                                                          AngelScript::asFUNCTION(as_set_bool),
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("int get_int(string &in)",
                                                          AngelScript::asFUNCTION(as_get_int),
                                                          AngelScript::asCALL_CDECL);
            ...
            m_script_engine.get()->RegisterGlobalFunction("float get_nozzle(string &in, int idx)",
                                                          AngelScript::asFUNCTION(as_get_nozzle),
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("void set_nozzle(string &in, float new_val, int idx)",
            ...
            m_script_engine.get()->RegisterGlobalFunction("bool get_custom_bool(int, string &in, bool &out)",
                                                          ...
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("void set_custom_bool(int, string &in, bool)",
                                                          AngelScript::asFUNCTION(as_set_custom_bool),
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("bool get_custom_int(int, string &in, int &out)",
                                                          AngelScript::asFUNCTION(as_get_custom_int),
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("void set_custom_int(int, string &in, int)",
                                                          AngelScript::asFUNCTION(as_set_custom_int),
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("bool get_custom_float(int, string &in, float &out)",
                                                          AngelScript::asFUNCTION(as_get_custom_float),
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("void set_custom_float(int, string &in, float)",
                                                          AngelScript::asFUNCTION(as_set_custom_float),
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("bool get_custom_string(int, string &in, string &out)",
                                                          AngelScript::asFUNCTION(as_get_custom_string),
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("void set_custom_string(int, string &in, string &in)",
            ...
            m_script_engine.get()->RegisterGlobalFunction("float get_computed_float(string &in)",
                                                          AngelScript::asFUNCTION(as_get_computed_float),
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("void back_initial_value(string &in)",
                                                          AngelScript::asFUNCTION(as_back_initial_value),
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("void back_custom_initial_value(int, string &in)",
                                                          AngelScript::asFUNCTION(as_back_custom_initial_value),
                                                          AngelScript::asCALL_CDECL);
            m_script_engine.get()->RegisterGlobalFunction("void ask_for_refresh()",
                                                          AngelScript::asFUNCTION(as_ask_for_refresh),
                                                          AngelScript::asCALL_CDECL);
        ...
        if (res < 0)
            throw CompileErrorException("Error, can't build the script for tab " + tab_key);
        ...
        // res = builder.AddSectionFromFile(ui_script_file.string().c_str()); //seems to be problematic on cyrillic locale
        {
            ...
            res = builder.AddSectionFromMemory(ui_script_file.string().c_str(), all_file.c_str(),
                                               (unsigned int) (all_file.length()), 0);
        }
        if (res < 0)
            throw CompileErrorException("Error, can't build the script for tab " + tab_key);
        ...
        BOOST_LOG_TRIVIAL(warning) << "Warning, can't find file script '" << ui_script_file.string()
                                   << "', is it needed? Even if something need it, it won't execute.";
