...
ObjectInfo::ObjectInfo(wxWindow *parent) : wxStaticBoxSizer(new wxStaticBox(parent, wxID_ANY, _L("Info")), wxVERTICAL)
{
    ...
    auto init_info_label = [parent, grid_sizer](wxStaticText **info_label, wxString text_label,
                                                wxSizer *sizer_with_icon = nullptr) {
...
void FreqChangedParams::msw_rescale()
{
    if (m_og)
        m_og->msw_rescale();
    for (auto &entry : m_og_other) entry.second->msw_rescale();

    for (auto btn : m_empty_buttons) btn->msw_rescale();
}
...
void FreqChangedParams::init()
{
    ...
    auto empty_widget = [this](wxWindow *parent) {
        ...
        auto btn   = new ScalableButton(parent, wxID_ANY, "mirroring_transparent.png", wxEmptyString, wxDefaultSize,
                ...
                m_wiping_dialog_button
                    ->Bind(wxEVT_BUTTON, ([parent](wxCommandEvent &e) {
                               ...
                               if (dlg.ShowModal() == wxID_OK) {
                                   ...
                                   (project_config.option<ConfigOptionFloats>("wiping_volumes_matrix"))
                                       ->set(std::vector<double>(matrix.begin(), matrix.end()));
                                   (project_config.option<ConfigOptionFloats>("wiping_volumes_extruders"))
                                       ->set(std::vector<double>(extruders.begin(), extruders.end()));
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    p->sizer_params->Add(p->frequently_changed_parameters->get_sizer(), 0, wxLEFT | wxEXPAND | wxALIGN_CENTER,
                         wxOSX ? 1 : margin_5);
    ...
    is_msw ? scrolled_sizer->Add(p->presets_panel, 0, wxEXPAND | wxLEFT, margin_5) :
    ...
    auto init_scalable_btn = [this](ScalableButton **btn, const std::string &icon_name,
    ...
    init_scalable_btn(&p->btn_send_gcode, "export_gcode",
                      _L("Send to printer") + " " + GUI::shortkey_ctrl_prefix() + "Shift+G");
    //    init_scalable_btn(&p->btn_eject_device, "eject_sd"       , _L("Remove device ") +
    //    GUI::shortkey_ctrl_prefix() + "T");
...
void Sidebar::update_all_preset_comboboxes()
{
    ...
    if (print_tech == ptFFF) {
        for (PlaterPresetComboBox *cb : p->combos_filament) cb->update();
    }
...
void Sidebar::update_presets(Preset::Type preset_type)
{
    ...
    case Preset::TYPE_FFF_PRINT: p->combo_print->update(); break;
...
void Sidebar::msw_rescale()
{
    ...
    for (PlaterPresetComboBox *combo : std::vector<PlaterPresetComboBox *>{p->combo_print, p->combo_sla_print,
                                                                           p->combo_sla_material, p->combo_printer})
        combo->msw_rescale();
...
void Sidebar::sys_color_changed()
{
    ...
    for (wxWindow *win : std::vector<wxWindow *>{this, p->sliced_info->GetStaticBox(), p->object_info->GetStaticBox(),
                                                 p->btn_reslice, p->btn_export_gcode})
    ...
    for (PlaterPresetComboBox *combo : p->combos_filament) combo->sys_color_changed();
...
void Sidebar::init_freq_params() { p->frequently_changed_parameters->init(); }
...
void Sidebar::show_info_sizer()
{
    ...
    if ((m_mode < comExpert && wxGetApp().app_config->get("objects_always_expert") != "1") || objects.empty() ||
        obj_idx < 0 || obj_idx == 1000 ||
    ...
    if (selection.is_single_volume()) {
        ...
        if (vol_idxs.size() != 1)
            ...
            // Like after try to delete last solid part in object, the object is selected in ObjectLIst when just a
            // part is still selected on Canvas see https://github.com/prusa3d/PrusaSlicer/issues/7408
            return;
    ...
    p->object_info->info_size->SetLabel(
    ...
    p->object_info->info_facets->SetLabel(
    ...
    if (p->plater->printer_technology() == ptSLA) {
        for (auto item : p->object_info->sla_hidden_items) item->Show(false);
    }
}
        ...
        if (p->plater->printer_technology() == ptSLA) {
            ...
            if (is_supports)
                new_label += format_wxstr("\n    - %s\n    - %s",
                                          _L_PLURAL("object", "objects", p->plater->model().objects.size()),
                                          _L("supports and pad"));

            wxString info_text = is_supports ?
                                     wxString::Format("%.2f \n%.2f \n%.2f",
                                                      (ps.objects_used_material + ps.support_used_material) / 1000,
            ...
            if (cfg->option("bottle_cost")->get_float() > 0.0 && cfg->option("bottle_volume")->get_float() > 0.0) {
                double material_cost = cfg->option("bottle_cost")->get_float() /
                                       cfg->option("bottle_volume")->get_float();
                str_total_cost = wxString::Format("%.3f", material_cost *
                                                              (ps.objects_used_material + ps.support_used_material) /
                                                              1000);
            }
            p->sliced_info->SetTextAndShow(siCost, str_total_cost, "Cost");

            wxString t_est = std::isnan(ps.estimated_print_time) ? "N/A" :
                                                                   get_time_dhms(float(ps.estimated_print_time));
            p->sliced_info->SetTextAndShow(siEstimatedTime, t_est, _L("Estimated printing time") + ":");

            p->plater->get_notification_manager()->set_slicing_complete_print_time(_utf8(
                    ...
                    if (filament_preset) {
                        ...
                        for (auto entry : ps.color_extruderid_to_used_filament) {
                            if (filament_id == entry.first) {
                                items_printed++;
                                new_label += "\n    - " + format_wxstr(_L("Color %1% at extruder %2%"), items_printed,
                                                                       (filament_id + 1));
                                total_length += entry.second;
                                info_text += wxString::Format("\n%.2f (%.2f)", entry.second / 1000,
                                                              total_length / 1000);
                            }
                        }
                        ...
                        if (items_printed == 0) {
                            ...
                            // ps.color_extruderid_to_used_filament.size()).str());
                            info_text += wxString::Format("\n%.2f", filament_volume * mm3_to_m);
                        } else {
                            new_label += "\n    - " + format_wxstr(_L("Color %1% at extruder %2%"),
                                                                   (items_printed + 1), (filament_id + 1));
                            info_text += wxString::Format("\n%.2f (%.2f)",
                                                          (filament_volume - total_length) * mm3_to_m,
                                                          filament_volume * mm3_to_m);
                        }
                    }
                }
            }
                ...
                if (ps.filament_stats.size() > 1 || ps.color_extruderid_to_used_weight.size() > 0) {
                    ...
                    for (auto [filament_id, filament_volume] : ps.filament_stats) {
                        ...
                        if (filament_preset) {
                            ...
                            double filament_density = filament_preset->config.opt_float("filament_density",
                                                                                        filament_id);
                            double crosssection     = 0.5 *
                                                  filament_preset->config.opt_float("filament_diameter", filament_id);
                            crosssection *= crosssection * PI;
                            ...
                            //    new_label += from_u8((boost::format("\n    - %1% %2%") % _utf8(L("Color")) % (i +
                            //    1)).str()); total_weight += ps.color_extruderid_to_used_weight[i].second; info_text
                            //    += (ps.color_extruderid_to_used_weight[i].second == 0 ? "\nN/A" :
                            //    wxString::Format("\n%.2f", ps.color_extruderid_to_used_weight[i].second / 1000))
                            //        + (total_weight == 0 ? " (N/A)" : wxString::Format(" (%.2f)", total_weight /
                            //        1000));
                            //}
                            if (spool_weight != 0.0)
                                has_spool = true;
                            ...
                            if (items_printed == 0) {
                                ...
                                if (spool_weight != 0.0)
                                    info_text += wxString::Format(" (%.2f)",
                                                                  filament_volume * mm3_to_g + spool_weight);
                            } else {
                                new_label += "\n    - " + format_wxstr(_L("Color %1% at extruder %2%"),
                                                                       (items_printed + 1), (filament_id + 1));
                                ...
                                if (spool_weight != 0.0)
                                    info_text += wxString::Format(" (%.2f)",
                                                                  (filament_volume - total_length) * mm3_to_g +
                                                                      spool_weight);
                            }
                        }
                    }
            ...
            if (ps.estimated_print_time.empty())
                ...
                if (auto it = ps.estimated_print_time_str.find(
                        static_cast<uint8_t>(PrintEstimatedStatistics::ETimeMode::Normal));
                    it != ps.estimated_print_time_str.end()) {
                    ...
                    assert(ps.estimated_print_time.find(static_cast<uint8_t>(
                               PrintEstimatedStatistics::ETimeMode::Normal)) != ps.estimated_print_time.end());
                    std::chrono::system_clock::time_point time_finished =
                        time_now + std::chrono::seconds(int32_t(ps.estimated_print_time.at(
                                       static_cast<uint8_t>(PrintEstimatedStatistics::ETimeMode::Normal))));
                    std::time_t timestamp_finished = std::chrono::system_clock::to_time_t(time_finished);
                    info_text += format_wxstr(_L(" (finished at %1%)"),
                                              std::put_time(std::localtime(&timestamp_finished), "%T"));

                    p->plater->get_notification_manager()
                        ->set_slicing_complete_print_time(_utf8("Estimated printing time: ") + it->second,
                                                          p->plater->is_sidebar_collapsed());
                }
                if (auto it = ps.estimated_print_time_str.find(
                        static_cast<uint8_t>(PrintEstimatedStatistics::ETimeMode::Stealth));
                    it != ps.estimated_print_time_str.end()) {
                    new_label += format_wxstr("\n   - %1%", _L("stealth mode"));
                    info_text += format_wxstr("\n%1%", short_time(it->second));

                    assert(ps.estimated_print_time.find(static_cast<uint8_t>(
                               PrintEstimatedStatistics::ETimeMode::Stealth)) != ps.estimated_print_time.end());
                    std::chrono::system_clock::time_point time_finished =
                        time_now + std::chrono::seconds(int32_t(ps.estimated_print_time.at(
                                       static_cast<uint8_t>(PrintEstimatedStatistics::ETimeMode::Stealth))));
                    std::time_t timestamp_finished = std::chrono::system_clock::to_time_t(time_finished);
            ...
            p->sliced_info->SetTextAndShow(siWTNumbetOfToolchanges,
                                           is_wipe_tower ? wxString::Format("%.d", ps.total_toolchanges) : "N/A");
...
void Sidebar::update_mode()
{
    ...
    p->object_list->get_sizer()->Show(m_mode > comSimple ||
                                      wxGetApp().app_config->get("objects_always_expert") == "1");
...
struct Plater::priv
{
    ...
    public:
        Jobs(priv *_m) : m(_m), m_pri{std::make_shared<NotificationProgressIndicator>(m->notification_manager.get())}
        {
                ...
                if (act.empty()) {
                    ...
                    if (res != wxID_CANCEL)
                        if (dialog.IsCheckBoxChecked()) {
                            ...
                            MessageDialog msg_dlg(mainframe, msg,
                                                  format_wxstr(_L("%1%: Don't ask me again"), SLIC3R_APP_NAME),
    ...
    bool replace_volume_with_stl(int             object_idx,
                                 int             volume_idx,
...
Plater::priv::priv(Plater *q, MainFrame *main_frame)
    ...
    , config(Slic3r::DynamicPrintConfig::new_from_defaults_keys(
          {// These keys are used by (at least) printconfig::min_object_distance
           "bed_shape", "bed_custom_texture", "bed_custom_model", "brim_width", "brim_width_interior",
           "brim_separation", "complete_objects", "parallel_objects_step", "complete_objects_sort",
           "complete_objects_one_skirt", "brim_per_object", "duplicate_distance", "draft_shield",
           "extruder_clearance_radius", "first_layer_extrusion_width", "init_z_rotate", "max_print_height",
           "perimeter_extrusion_width", "extrusion_width", "skirts", "skirt_brim", "skirt_distance",
           "skirt_distance_from_brim", "skirt_extrusion_width", "skirt_height", "first_layer_extrusion_spacing",
           "perimeter_extrusion_spacing", "extrusion_spacing", "variable_layer_height", "nozzle_diameter",
           "single_extruder_multi_material", "wipe_tower", "wipe_tower_brim_width", "wipe_tower_rotation_angle",
           "wipe_tower_width", "wipe_tower_x", "wipe_tower_y", "extruder_colour", "filament_colour",
           "material_colour", "printer_model", "printer_technology",
           // These values are necessary to construct SlicingParameters by the Canvas3D variable layer height editor.
           "layer_height", "first_layer_height", "min_layer_height", "max_layer_height", "perimeters",
           "perimeter_extruder", "fill_density", "infill_extruder", "raft_layers", "support_material",
           "support_material_extruder", "support_material_interface_extruder", "support_material_contact_distance",
           "top_solid_layers", "z_step"}))
    ...
    , m_project_filename(wxEmptyString)
{
    ...
    preview = new Preview(q, bed, &model, config, &background_process, &gcode_result,
    ...
    if (wxGetApp().is_editor()) {
        ...
        view3D_canvas->Bind(EVT_GLCANVAS_ADAPTIVE_LAYER_HEIGHT_PROFILE, [this](HeightProfileAdaptiveEvent &evt) {
    ...
    // Preview events:
    preview->get_wxglcanvas()->Bind(EVT_GLCANVAS_QUESTION_MARK,
                                    [](SimpleEvent &) { wxGetApp().keyboard_shortcuts(); });
    ...
    preview->get_wxglcanvas()->Bind(EVT_GLCANVAS_JUMP_TO,
                                    [this](wxKeyEvent &evt) { preview->jump_layers_slider(evt); });
    ...
    preview->get_wxglcanvas()->Bind(EVT_GLCANVAS_EDIT_COLOR_CHANGE,
                                    [this](wxKeyEvent &evt) { preview->edit_layers_slider(evt); });
    ...
    set_current_panel(wxGetApp().is_editor() ? static_cast<wxTitledPanel *>(view3D) :
                                               static_cast<wxTitledPanel *>(preview));
    if (wxGetApp().is_gcode_viewer())
...
#ifdef _WIN32
    // Register an USB HID (Human Interface Device) attach event. evt contains Win32 path to the USB device containing
    // VID, PID and other info. This event wakes up the Mouse3DController's background thread to enumerate HID
    // devices, if the VID of the callback event is one of the 3D Mouse vendors (3DConnexion or Logitech).
        ...
        this->q->Bind(EVT_REMOVABLE_DRIVE_EJECTED, [this](RemovableDriveEjectEvent &evt) {
            if (evt.data.second) {
                ...
                notification_manager->push_notification(NotificationType::CustomNotification,
                                                        ...
                                                        format(_L("Ejecting of device %s(%s) has failed."),
                                                               evt.data.first.name, evt.data.first.path));
            }
        });
    ...
    this->q->Bind(EVT_LOAD_MODEL_OTHER_INSTANCE, [this](LoadFromOtherInstanceEvent &evt) {
        ...
        for (size_t i = 0; i < evt.data.size(); ++i) { input_files.push_back(from_u8(evt.data[i].string())); }
...
void Plater::priv::update(unsigned int flags)
{
    ...
    if (force_background_processing_restart)
        ...
        update_status = this->update_background_process(false,
                                                        flags & (unsigned int)
                                                                    UpdateParams::POSTPONE_VALIDATION_ERROR_MESSAGE);
...
void Plater::priv::collapse_sidebar(bool collapse)
{
    ...
    std::string new_tooltip = collapse ? _utf8(L("Expand sidebar")) : _utf8(L("Collapse sidebar"));
                                             ...
                                             bool                         imperial_units /* = false*/)
{
    ...
    bool one_by_one =
        input_files.size() == 1 ||
        printer_technology ==
            ptSLA; // || nozzle_dmrs->size() <= 1; // removed by bb (toa llow multi-import on a single extruder printer.
    if (!one_by_one) {
    ...
    for (size_t i = 0; i < input_files_size; ++i) {
        ...
        // on Windows thus it assignes full path to names of loaded objects.
        path.make_preferred();
#else  // _WIN32
                ...
                if (load_config) {
                    if (!config.empty()) {
                        ...
                        preset_bundle->load_config_model(filename.string(), std::move(config));
                        {
                            ...
                            auto update_selected_preset_visibility = [](PresetCollection &        presets,
                            ...
                            if (!names.empty()) {
                                std::string notif_text =
                                    into_u8(format_wxstr(_L_PLURAL("The preset below was temporarily installed on "
                                                                   "the active instance of %1%",
                                                                   "The presets below were temporarily installed on "
                                                                   "the active instance of %1%",
                                                                   names.size()),
                                                         SLIC3R_APP_NAME)) +
                                    ":";
                                for (std::string &name : names) notif_text += "\n - " + name;
                                notification_manager->push_notification(
                                    NotificationType::CustomNotification,
            ...
            if (!is_project_file) {
                ...
                if (imperial_units)
                            ...
                            dlg(q,
                                format_wxstr(
                                    _L_PLURAL(
                                        ...
                                        model.objects.size()),
                                    from_path(filename), SLIC3R_APP_NAME) +
                                    "\n",
                    ...
                    convert_model_if(model, answer_convert_from_meters == wxID_YES);
                           ...
                           false) { // don't do that, as it can be annoying (but usa).
                ...
                if (model.looks_like_multipart_object()) {
                    ...
                    if (msg_dlg.ShowModal() == wxID_YES) {
                        ...
                        wxGetApp().app_config->get("objects_always_expert") == "0") &&
                       (type_3mf || type_any_amf) && model_has_advanced_features(model)) {
                // wxMessageDialog msg_dlg(q, _L("This file cannot be loaded in a simple mode. Do you want to switch
                // to an advanced mode?")+"\n",
                ...
                for (auto obj : model.objects)
                    if (obj->volumes.size() > 1) {
                        Slic3r::GUI::show_error(nullptr,
                                                format_wxstr(_L("You can't to add the object(s) from %s because of "
                                                                "one or some of them is(are) multi-part"),
                                                             from_path(filename)));
                        return obj_idxs;
                    }
            }
    ...
    if (!obj_idxs.empty() && view3D != nullptr) {
        ...
        selection.clear();
        for (size_t idx : obj_idxs) { selection.add_object((unsigned int) idx, false); }

        if (view3D->get_canvas3d()->get_gizmos_manager().is_enabled())
            // this is required because the selected object changed and the flatten on face an sla support gizmos need
...
std::vector<size_t> Plater::priv::load_model_objects(const ModelObjectPtrs &model_objects, bool allow_negative_z)
{
    ...
    for (ModelObject *model_object : model_objects) {
        ...
        for (size_t i = 0; i < object->instances.size(); ++i) {
            ...
            if (max_ratio > 10000) {
                // the size of the object is too big -> this could lead to overflow when moving to clipper
                // coordinates, so scale down the mesh
    ...
    for (const size_t idx : obj_idxs) { wxGetApp().obj_list()->add_object_to_list(idx); }

    update();
    // Update InfoItems in ObjectList after update() to use of a correct value of the GLCanvas3D::is_sinking(),
    // which is updated after a view3D->reload_scene(false, flags & (unsigned
    // int)UpdateParams::FORCE_FULL_SCREEN_REFRESH) call
    for (const size_t idx : obj_idxs) wxGetApp().obj_list()->update_info_items(idx);
...
wxString Plater::priv::get_export_file(GUI::FileType file_type)
{
    ...
    wxFileDialog dlg(q, dlg_title,
                     ...
                     from_path(output_file.filename()), wildcard,
...
unsigned int Plater::priv::update_background_process(bool force_validation, bool postpone_error_messages)
{
    ...
    if (invalidated != Print::APPLY_STATUS_UNCHANGED && was_running && !this->background_process.running() &&
        ...
        evt.SetInt(-1);
        // Post the "canceled" callback message, so that it will be processed after any possible pending status bar
        // update messages.
...
bool Plater::priv::replace_volume_with_stl(int             object_idx,
                                           int             volume_idx,
                                           const fs::path &new_path,
                                           const wxString &snapshot)
{
    ...
    new_volume->translate(new_volume->get_transformation().get_matrix(true) *
...
void Plater::priv::set_current_panel(wxTitledPanel *panel)
{
    ...
    if (current_panel == view3D) {
        ...
        if (wxGetApp().is_editor()) {
            ...
            bool model_fits         = view3D->get_canvas3d()->check_volumes_outside_state() !=
                              ModelInstancePVS_Partly_Outside;

            if (!model.objects.empty() && !export_in_progress && model_fits) {
                // check if already slicing
                bool already_running = this->background_process.state() ==
                                           BackgroundSlicingProcess::State::STATE_RUNNING ||
                                       this->background_process.state() ==
                                           BackgroundSlicingProcess::State::STATE_STARTED;
                if (!already_running) {
...
void Plater::priv::on_slicing_update(SlicingStatusEvent &evt)
{
        ...
        if (evt.status.percent >= -1) {
            ...
            if (evt.status.args.empty()) {
                notification_manager
                    ->set_slicing_progress_percentage(evt.status.main_text.empty() ? "" : _utf8(evt.status.main_text),
                                                      evt.status.percent / 100.f,
                                                      0 == (evt.status.flags &
                                                            PrintBase::SlicingStatus::FlagBits::SECONDARY_STATE));
            } else {
                auto formatter = boost::format(_utf8(evt.status.main_text));
                for (std::string &arg : evt.status.args) formatter = formatter % _utf8(arg);
                notification_manager
                    ->set_slicing_progress_percentage(formatter.str(), evt.status.percent / 100.f,
                                                      0 == (evt.status.flags &
                                                            PrintBase::SlicingStatus::FlagBits::SECONDARY_STATE));
            }
        }
    ...
    if (evt.status.flags &
        ...
        // Update the SLA preview. Only called if not RELOAD_SLA_SUPPORT_POINTS, as the block above will refresh the
        // preview anyways.
        this->preview->reload_print();
    }
...
void Plater::priv::actualize_object_warnings(const PrintBase &print)
{
    ...
    for (const ModelObject *object : print.model().objects) { ids.push_back(object->id()); }
    std::sort(ids.begin(), ids.end());
    ...
    if (obj_idx == -1) {                     // no one or several object are selected
        ...
        if (printer_technology == ptSLA)
            ...
            menu               = is_some_full_instances ? menus.object_menu() :
                                 is_part                ? menus.part_menu() :
                                                          menus.multi_selection_menu();
        }
    }
...
void Plater::priv::update_preview_bottom_toolbar() { preview->update_bottom_toolbar(); }
...
bool Plater::priv::can_scale_to_print_volume() const
{
    ...
    return !view3D->get_canvas3d()->get_selection().is_empty() &&
...
bool Plater::priv::layers_height_allowed() const
{
    ...
    return 0 <= obj_idx && obj_idx < (int) model.objects.size() &&
           model.objects[obj_idx]->bounding_box().max.z() > SINKING_Z_THRESHOLD &&
           config->opt_bool("variable_layer_height") && view3D->is_layers_editing_allowed();
}

bool Plater::priv::can_mirror() const { return get_selection().is_from_single_instance(); }

bool Plater::priv::can_replace_with_stl() const { return get_selection().get_volume_idxs().size() == 1; }
...
bool Plater::priv::can_reload_from_disk() const
{
    ...
    struct SelectedVolume
    {
        ...
        bool operator<(const SelectedVolume &other) const
        {
            return (object_idx < other.object_idx) ||
                   ((object_idx == other.object_idx) && (volume_idx < other.volume_idx));
        }
        bool operator==(const SelectedVolume &other) const
        {
            return (object_idx == other.object_idx) && (volume_idx == other.volume_idx);
...
{
    if (m_ui_jobs.is_any_running() || q->canvas3D()->get_gizmos_manager().is_in_editing_mode())
        return false;
...
bool Plater::priv::can_arrange() const { return !model.objects.empty() && !m_ui_jobs.is_any_running(); }
...
{
    // Cache this value, so that the callbacks from the RemovableDriveManager may repeat that value when calling
    // show_action_buttons().
...
void Plater::priv::take_snapshot(const std::string &snapshot_name, const UndoRedo::SnapshotType snapshot_type)
{
    ...
    // This is a workaround until we refactor the Wipe Tower position / orientation to live solely inside the Model,
    ...
    if (snapshot_type == UndoRedo::SnapshotType::ProjectSeparator &&
        wxGetApp().app_config->get("clear_undo_redo_stack_on_new_project") == "1")
    ...
    if (snapshot_type == UndoRedo::SnapshotType::LeavingGizmoWithAction) {
        // Filter all but the last UndoRedo::SnapshotType::GizmoAction in a row between the last
        // UndoRedo::SnapshotType::EnteringGizmo and UndoRedo::SnapshotType::LeavingGizmoWithAction. The remaining
        // snapshot will be renamed to a more generic name, depending on what gizmo is being left.
...
void Plater::priv::undo_redo_to(std::vector<UndoRedo::Snapshot>::const_iterator it_snapshot)
{
    ...
    if (printer_technology_changed) {
        // Switching the printer technology when jumping forwards / backwards in time. Switch to the last active
        // printer profile of the other type.
        ...
        if (!wxGetApp().check_and_save_current_preset_changes(
                ...
                //            format_wxstr(_L("%1% printer was active at the time the target Undo / Redo snapshot was
                //            taken. Switching to %1% printer requires reloading of %1% presets."), s_pt)))
                format_wxstr(
    ...
    if (it_snapshot->timestamp < this->undo_redo_stack().active_snapshot_time() ?
        ...
        if (printer_technology_changed) {
            ...
            // FIXME Why are we reloading the whole preset bundle here? Please document. This is fishy and it is
            // unnecessarily expensive.
            // Anyways, don't report any config value substitutions, they have been already reported to the user at
            ...
            // Object_list::update_and_show_object_settings_item(), but the Object list still keeps pointer to the old
            // Model. Avoid a crash by removing selection first.
        ...
        // This is a workaround until we refactor the Wipe Tower position / orientation to live solely inside the
        // Model, not in the Print config.
        if (this->printer_technology == ptFFF) {
...
void Plater::priv::update_after_undo_redo(const UndoRedo::Snapshot &snapshot, bool /* temp_snapshot_was_taken */)
{
    ...
    // Update volumes from the deserializd model, always stop / update the background processing (for both the SLA and
    // FFF technologies).
    ...
    // Release old snapshots if the memory allocated is excessive. This may remove the top most snapshot if jumping to
    // the very first snapshot.
    // if (temp_snapshot_was_taken)
    // Release the old snapshots always, as it may have happened, that some of the triangle meshes got deserialized
    // from the snapshot, while some triangle meshes may have gotten released from the scene or the background
    // processing, therefore now being calculated into the Undo / Redo stack size.
    this->undo_redo_stack().release_least_recently_used();
    // YS_FIXME update obj_list from the deserialized model (maybe store ObjectIDs into the tree?) (no selections at
    // this point of time)
    ...
    if (wxGetApp().get_mode() == comSimple && model_has_advanced_features(this->model) &&
        wxGetApp().app_config->get("objects_always_expert") != "1") {
        // If the user jumped to a snapshot that require user interface with advanced features, switch to the advanced
        // mode without asking. There is a little risk of surprising the user, as he already must have had the
        // advanced or expert mode active for such a snapshot to be taken.
        Slic3r::GUI::wxGetApp().save_mode(comAdvanced);
        view3D->set_as_dirty();
    }

    // this->update() above was called with POSTPONE_VALIDATION_ERROR_MESSAGE, so that if an error message was
    // generated when updating the back end, it would not open immediately, but it would be saved to be show later.
    // Let's do it now. We do not want to display the message box earlier, because on Windows & OSX the message box
    // takes over the message queue pump, which in turn executes the rendering function before a full update after the
    // Undo / Redo jump.
    ...
    if (int saved_project = p->save_project_if_dirty(
        ...
        wxString header = _L("Creating a new project while some presets are modified.") + "\n" +
                          (saved_project == wxID_YES ?
                               ...
                               _L("You can keep presets modifications to the new project, discard them or save "
                ...
                wxDEFAULT_DIALOG_STYLE)
{
    ...
    const wxString choices[] = {_L("Open as project"), _L("Import geometry only"), _L("Import config only")};
                    ...
                    if (wxGetApp().app_config->get("show_drop_project_dialog") == "1") {
                        ...
                        load_type = static_cast<LoadType>(
                            std::clamp(std::stoi(wxGetApp().app_config->get("drop_project_action")),
                                       ...
                                       static_cast<int>(LoadType::LoadConfig)));
                }
            } else
...
bool Plater::is_any_job_running() const { return p->m_ui_jobs.is_any_running(); }
...
Preview::ForceState Plater::get_force_preview() { return p->preview->get_force_state(); }
...
void Plater::reset_with_confirm()
{
    if (get_app_config()->get("default_action_delete_all") == "1") {
        if (!p->model.objects.empty()) {
            MessageDialog /*wxMessageDialog*/ dialog(static_cast<wxWindow *>(this),
                                                     _L("All objects will be removed, continue?"),
            ...
            if (MessageDialog(static_cast<wxWindow *>(this), _L("Create a new project?"),
                              wxString(SLIC3R_APP_NAME) + " - " + _L("Delete all"),
                              wxYES | wxCANCEL | wxYES_DEFAULT | wxCENTRE)
                    .ShowModal() == wxID_YES)
                new_project();
        }
    } else {
...
void Plater::decrease_instances(size_t num)
{
    ...
    if (model_object->instances.size() > num) {
        for (size_t i = 0; i < num; ++i) model_object->delete_last_instance();
        p->update();
        // Delete object from Sidebar list. Do it after update, so that the GLScene selection is updated with the
        // modified model.
...
void Plater::set_number_of_copies(/*size_t num*/)
{
    ...
    const int num = GetNumberFromUser(" ", _L("Enter the number of copies:"), _L("Copies of the selected object"),
                                      model_object->instances.size(), 0, 1000, this);
    if (num < 0)
        return;
...
void Plater::convert_unit(ConversionType conv_type)
{
    ...
    TakeSnapshot snapshot(this,
                          conv_type == ConversionType::CONV_FROM_INCH  ? _L("Convert from imperial units") :
    ...
    if (volume_idxs.empty()) {
        for (size_t i = 0; i < objects.size(); ++i) selection.add_object((unsigned int) (last_obj_idx - i), i == 0);
    } else {
        for (int vol_idx : volume_idxs) selection.add_volume(last_obj_idx, vol_idx, 0, false);
    }
}
...
void Plater::cut(size_t obj_idx, size_t instance_idx, coordf_t z, ModelObjectCutAttributes attributes)
{
    ...
    for (size_t i = 0; i < new_objects.size(); ++i) selection.add_object((unsigned int) (last_id - i), i == 0);
}
...
void Plater::export_gcode(bool prefer_removable)
{
    ...
    if (p->get_config("check_material_export") == "1") {
        ...
        if (printer_technology() == ptFFF) {
            const ConfigOptionStrings *filaments = fff_print().full_print_config().opt<ConfigOptionStrings>(
                "filament_settings_id");
            assert(filaments->size() == fff_print().config().filament_type.size());
            for (int i = 0; i < filaments->size(); i++) {
                str_material += "\n" + format(_L("'%1%' of type %2%"), filaments->get_at(i),
                                              fff_print().config().filament_type.get_at(i));
            }
        } else if (printer_technology() == ptSLA) {
            str_material = format(_L(" resin '%1%'"),
                                  sla_print().full_print_config().opt_string("sla_material_settings_id"));
        }
        MessageDialog dlg(this,
                          format_wxstr(_L("You will export the file with the material profile(s): %1%"), str_material),
                          _L("Checking your material"), wxOK | wxOK_DEFAULT | wxCANCEL | wxICON_WARNING);
        if (dlg.ShowModal() != wxID_OK)
            return;
    }
    ...
    try {
        // Update the background processing, so that the placeholder parser will get the correct values for the ouput
        // file template. Also if there is something wrong with the current configuration, a pop-up dialog will be
        // shown and the export will not be performed.
    ...
    std::string start_dir = appconfig.get_last_output_dir(default_output_file.parent_path().string(),
                                                          prefer_removable);
    if (prefer_removable) {
        // Returns a path to a removable media if it exists, prefering start_dir. Update the internal removable drives
        // database.
    ...
    fs::path output_path;
    {
        ...
        wxFileDialog dlg(this,
                         (printer_technology() == ptFFF) ? _L("Save G-code file as:") : _L("Save SL1 / SL1S file as:"),
                         start_dir, from_path(default_output_file.filename()),
    ...
    if (!output_path.empty()) {
        ...
        // is_path_on_removable_drive() is called with the "true" parameter to update its internal database as the
        // user may have shuffled the external drives while the dialog was open.
...
OptionForExportPlatter::OptionForExportPlatter(wxWindow *parent) : wxPanel(parent)
{
...
void Plater::export_platter()
{
    ...
    dlg.SetExtraControlCreator([](wxWindow *parent) -> wxWindow * { return new OptionForExportPlatter(parent); });
    ...
    if (dlg.GetFilterIndex() == 0) {
        ...
        if (dlg.GetFilterIndex() == 1) {
            ...
            p->view3D->get_canvas3d()->render_thumbnail(thumbnail_data, THUMBNAIL_SIZE_3MF.first,
                                                        THUMBNAIL_SIZE_3MF.second, thumbnail_params, collection,
                                                        Camera::EType::Ortho);

            // store 3mf
            bool ret = Slic3r::store_3mf(path_u8.c_str(), &model_to_save, &full_config,
                                         OptionStore3mf{}
                                             .set_fullpath_sources(
                                                 wxGetApp().app_config->get("export_sources_full_pathnames") == "1")
            ...
            // store amf
            bool ret = Slic3r::store_amf(path_u8, &model_to_save, &full_config,
                                         OptionStoreAmf{}
                                             .set_fullpath_sources(
                                                 wxGetApp().app_config->get("export_sources_full_pathnames") == "1")
    ...
    if (p->printer_technology == ptFFF) {
        if (selection_only) {
            ...
            if (selection.get_mode() == Selection::Instance)
                mesh = selection.is_single_full_object() ?
                           mesh_to_export(*model_object, -1) :
                           mesh_to_export(*model_object, selection.get_instance_idx());
            else {
            ...
            for (const ModelObject *o : p->model.objects) { mesh.merge(mesh_to_export(*o, -1)); }
        }
    } else {
        ...
        for (const SLAPrintObject *object : objects) {
            ...
            for (const SLAPrintObject::Instance &obj_instance : obj_instances) {
                auto it = std::find_if(model_object->instances.begin(), model_object->instances.end(),
                                       [&obj_instance](const ModelInstance *mi) {
                                           return mi->id() == obj_instance.instance_id;
                                       });
...
void Plater::export_amf()
{
    ...
    if (Slic3r::store_amf(path_u8, &p->model, export_config ? &cfg : nullptr,
                          OptionStoreAmf{}.set_fullpath_sources(full_pathnames))) {
        // Success
...
bool Plater::export_3mf(const boost::filesystem::path &output_path)
{
    ...
    p->generate_thumbnail(thumbnail_data, THUMBNAIL_SIZE_3MF.first, THUMBNAIL_SIZE_3MF.second, thumbnail_params,
                          Camera::EType::Ortho);
    bool ret =
        Slic3r::store_3mf(path_u8.c_str(), &p->model, &cfg,
                          OptionStore3mf{}.set_fullpath_sources(full_pathnames).set_thumbnail_data(&thumbnail_data));
    if (ret) {
        // Success
...
void Plater::send_gcode()
{
    ...
    DynamicPrintConfig *physical_printer_config = wxGetApp()
                                                      .preset_bundle->physical_printers.get_selected_printer_config();
...
bool Plater::update_filament_colors_in_full_config()
{
    ...
    for (const std::string &filament_preset : filament_presets)
        filament_colors.push_back(
            filaments.find_preset(filament_preset, true)->config.opt_string("filament_colour", (unsigned) 0));
...
void Plater::on_config_change(const DynamicConfig &config)
{
    ...
    for (const std::string &opt_key : diff) {
        ...
        if (opt_key == "printer_technology") {
            ...
            update_scheduled = true;
            if (p->config->option<ConfigOptionStrings>("filament_colour")->size() <
                config.option<ConfigOptionStrings>(opt_key)->size()) {
                const std::vector<std::string> filament_presets = wxGetApp().preset_bundle->filament_presets;
                const PresetCollection &       filaments        = wxGetApp().preset_bundle->filaments;
                std::vector<std::string>       filament_colors;
                filament_colors.reserve(filament_presets.size());
                ...
                for (const std::string &filament_preset : filament_presets)
                    filament_colors.push_back(filaments.find_preset(filament_preset, true)
...
{
    // Fill in the printer model key with something which cannot possibly be valid, so that Plater::on_config_change()
    // will update the print bed once a new Printer profile config is loaded.
...
std::vector<std::string> Plater::get_extruder_colors_from_plater_config(const GCodeProcessorResult *const result) const
{
    if (wxGetApp().is_gcode_viewer() && result != nullptr)
        ...
        const std::vector<std::string> &filament_colours = (p->config->option<ConfigOptionStrings>("filament_colour"))
                                                               ->get_values();
...
bool Plater::set_printer_technology(PrinterTechnology printer_technology)
{
    ...
    if (printer_technology == ptSLA) {
        for (ModelObject *model_object : p->model.objects) { model_object->ensure_on_bed(); }
    }
...
{}
Plater::TakeSnapshot::TakeSnapshot(Plater *               plater,
...
bool Plater::PopupMenu(wxMenu *menu, const wxPoint &pos)
{
    ...
    if (!m_tracking_popup_menu_error_message.empty()) {
        ...
        // The menus likely sends out some commands, so we may be safer if the dialog is shown after the menu command
        // is processed.
