...
public:
    ...
    struct StateWithWarnings : public StateWithTimeStamp
    {
        ...
        {
            for (auto &w : warnings) w.current = false;
        }
...
//            assert(m_state[i].state != STARTED);
    ...
    template<typename CancelationCallback> bool invalidate_all(CancelationCallback cancel)
    {
        ...
        if (invalidated) {
            ...
            // Now the worker thread should be stopped, therefore it cannot write into the warnings field.
            // It is safe to modify the warnings.
            for (size_t i = 0; i < COUNT; ++i) m_state[i].mark_warnings_non_current();
            m_step_active = -1;
        }
        return invalidated;
    }
...
public:
    ...
    virtual std::pair<PrintValidationError, std::string> validate(std::string *warning = nullptr) const
    {
    ...
    // If the filename_base is set, it is used as the input for the template processing. In that case the path is
    // expected to be the directory (may be empty). If filename_set is empty, than the path may be a file or
    // directory. If it is a file, then the macro will not be processed.
...
protected:
    ...
    // Wrapper around this->throw_if_canceled(), so that throw_if_canceled() may be passed to a function without
    // making throw_if_canceled() public.
...
protected:
    ...
    void active_step_add_warning(PrintStateBase::WarningLevel warning_level,
                                 ...
                                 int                          message_id = 0)
    {
        std::pair<PrintStepEnum, bool> active_step = m_state.active_step_add_warning(warning_level, message,
                                                                                     message_id, this->state_mutex());
