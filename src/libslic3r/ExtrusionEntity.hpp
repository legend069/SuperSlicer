...
enum ExtrusionRole : uint8_t {
    erNone, // erNone needs to be 0 (for cooling and everything that use the role as index). It must not be used.
            // Please use erTravel for not-extrusion role.
    erPerimeter,
    erExternalPerimeter,
    erOverhangPerimeter,
    erInternalInfill,
...
{
    return role == erPerimeter || role == erExternalPerimeter || role == erThinWall || role == erOverhangPerimeter;
}

inline bool is_infill(ExtrusionRole role)
{
    return role == erBridgeInfill || role == erInternalBridgeInfill || role == erInternalInfill ||
           role == erSolidInfill || role == erTopSolidInfill || role == erIroning;
}

inline bool is_solid_infill(ExtrusionRole role)
{
    return role == erBridgeInfill || role == erInternalBridgeInfill || role == erSolidInfill ||
           role == erTopSolidInfill || role == erIroning;
}
...
//    virtual void use(ExtrusionMultiPath3D &multipath3D) { ExtrusionEntity &entity = multipath3D;
//    default_use(entity); }; virtual void use(ExtrusionLoop &loop) { ExtrusionEntity &entity = loop;
//    default_use(entity); }; virtual void use(ExtrusionEntityCollection &collection) { ExtrusionEntity &entity =
//    collection;  default_use(entity); };
//};
...
//    virtual void use(const ExtrusionMultiPath &multipath) { const ExtrusionEntity &entity = multipath;
//    default_use(entity); }; virtual void use(const ExtrusionMultiPath3D &multipath3D) { const ExtrusionEntity
//    &entity = multipath3D; default_use(entity); }; virtual void use(const ExtrusionLoop &loop) { const
//    ExtrusionEntity &entity = loop; default_use(entity); }; virtual void use(const ExtrusionEntityCollection
//    &collection) { const ExtrusionEntity &entity = collection;  default_use(entity); };
//};
...
public:
    ...
    {
        Polygons out;
        this->polygons_covered_by_spacing(out, spacing_ratio, scaled_epsilon);
        return out;
    }
    ...
    virtual PolylinesOrArcs as_polylines() const
    {
        PolylinesOrArcs dst;
...
public:
    ...
    ExtrusionPath(const ExtrusionPath &rhs)
        ...
        , height(rhs.height)
        , m_role(rhs.m_role)
        , ExtrusionEntity(rhs.can_reverse())
    {
        assert(mm3_per_mm == mm3_per_mm);
        assert(width == width);
        assert(height == height);
    ...
    ExtrusionPath(const PolylineOrArc &polyline, const ExtrusionPath &rhs)
        : polyline(polyline)
        , mm3_per_mm(rhs.mm3_per_mm)
        , width(rhs.width)
        , height(rhs.height)
    ...
    ExtrusionPath(PolylineOrArc &&polyline, const ExtrusionPath &rhs)
...
public:
    ...
    ExtrusionPath3D(ExtrusionRole role, double mm3_per_mm, float width, float height, bool can_reverse)
        : ExtrusionPath(role, mm3_per_mm, width, height, can_reverse){/*std::cout << "new path3D++\n";*/};
    ExtrusionPath3D(const ExtrusionPath &rhs) : ExtrusionPath(rhs)
    { /*std::cout << "new path3D from path "<<size()<<"?"<<z_offsets.size()<<"\n";*/
    }
    ExtrusionPath3D(ExtrusionPath &&rhs) : ExtrusionPath(rhs)
    { /*std::cout << "new path3D from path " << size() << "?" << z_offsets.size()<<"\n";*/
    }
...
public:
    ...
    Polygons polygons_covered_by_width(const float scaled_epsilon = 0.f) const override
    {
        Polygons out;
        this->polygons_covered_by_width(out, scaled_epsilon);
        return out;
    }
    Polygons polygons_covered_by_spacing(const float spacing_ratio, const float scaled_epsilon) const override
    {
        Polygons out;
        this->polygons_covered_by_spacing(out, spacing_ratio, scaled_epsilon);
        return out;
    ...
    void collect_polylines(PolylinesOrArcs &dst) const override
    {
        PolylineOrArc pl = this->as_polyline();
        if (!pl.empty())
    ...
    void collect_points(Points &dst) const override
    {
        ...
        for (const ExtrusionPath &p : this->paths) append(dst, p.polyline.get_points());
    }
...
<<<<<<< HEAD
    ...
    ExtrusionLoop(const ExtrusionPaths &paths, ExtrusionLoopRole role = elrDefault)
        : paths(paths), m_loop_role(role), ExtrusionEntity(false)
    {
    ...
    ExtrusionLoop(ExtrusionPaths &&paths, ExtrusionLoopRole role = elrDefault)
        : paths(std::move(paths)), m_loop_role(role), ExtrusionEntity(false)
    {
        assert(this->first_point().coincides_with_epsilon(this->paths.back().polyline.back()));
    }
    ExtrusionLoop(const ExtrusionPath &path, ExtrusionLoopRole role = elrDefault)
        : m_loop_role(role), ExtrusionEntity(false)
    {
...
public:
    ...
    {
        if (path.role() == erGapFill && !_with_gap_fill)
            return;
...
public:
    ...
    {
        path.mm3_per_mm *= _flow_mult;
        path.width *= _flow_mult;
    }
    void use(ExtrusionPath3D &path3D) override
    {
        path3D.mm3_per_mm *= _flow_mult;
        path3D.width *= _flow_mult;
    }
...
struct LoopAssertVisitor : public ExtrusionVisitorRecursiveConst
{
    ...
    virtual void use(const ExtrusionLoop &loop) override
    {
        ...
        for (auto it = loop.paths.begin(); it != loop.paths.end(); ++it) { assert(it->length() > SCALED_EPSILON); }
