...
def main():
    ...
    for operation in operations:
        ...
        if ui_dir:
            test_file = os.path.join(ui_dir, "extruder.ui")
            if not os.path.isfile(test_file):
                # fallback to 'default' subdir
                fallback_dir = os.path.join(ui_dir, "default")
                print(f"extruder.ui not found, trying fallback: {fallback_dir}")
                if os.path.isfile(os.path.join(fallback_dir, "extruder.ui")):
                    ui_dir = fallback_dir
                else:
                    print("Warning: extruder.ui not found in either location!")
                    ui_dir = None

        if ui_dir:
            new_data = parse_ui_file(os.path.join(ui_dir, "extruder.ui"))
            new_data.extend(parse_ui_file(os.path.join(ui_dir, "extruder.ui")))
            new_data.extend(parse_ui_file(os.path.join(ui_dir, "filament.ui")))
            new_data.extend(parse_ui_file(os.path.join(ui_dir, "milling.ui")))
            new_data.extend(parse_ui_file(os.path.join(ui_dir, "print.ui")))
            new_data.extend(parse_ui_file(os.path.join(ui_dir, "printer_fff.ui")))
            new_data.extend(parse_ui_file(os.path.join(ui_dir, "printer_sla.ui")))
            new_data.extend(parse_ui_file(os.path.join(ui_dir, "sla_material.ui")))
            new_data.extend(parse_ui_file(os.path.join(ui_dir, "sla_print.ui")))
